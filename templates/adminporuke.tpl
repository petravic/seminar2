										...
										<div>
											<table id="tablica">
												{foreach $poruke as $index => $poruka}
													<tr><td>{$poruka.naslov}</td><td>{$poruka.username}</td><td>{$poruka.datum}</td><td>{$poruka.email}</td><td><button id="{$index}" class="brisanje" type="button">Obriši</button></td></tr>
													<tr><td colspan="5"><textarea disabled>{$poruka.tekst}</textarea></td></tr>
												{/foreach}
											</table>
											<br />
										</div>
