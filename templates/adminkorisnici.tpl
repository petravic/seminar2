										...
										<div>
											<table id="tablica">
												...
												{foreach $korisnici as $index => $korisnik}
													<tr><td>{$index}</td><td>{$korisnik.username}</td><td>{$korisnik.email}</td><td>{$korisnik.bonus}</td><td>{$korisnik.admin}</td><td>{if $korisnik.admin == DA && $korisnik.id == 1}X{else if $korisnik.admin == DA}<button id="{$korisnik.id}" class="remove" type="button">Ukloni ovlasti</button>{else}<button id="{$korisnik.id}" class="admin" type="button">Dodaj ovlasti</button>{/if}</td><td><button id="{$korisnik.id}" class="brisanje" type="button">Obriši</button></td></tr>
												{/foreach}
											</table>
											<br />
										</div>
